	...
	<Wrappper>
		<div class="my-auto h-14 md:h-16 flex justify-between w-full">
					...
					<a href="/">
						<h3
							class="text-primary {isBgTransparent
								? 'dark:text-white'
								: 'dark:text-text'} font-montserrat  text-sm md:text-lg z-0 transition-colors duration-500"
								style="color: #005bb5;"
						>
						 Srikanth
						</h3>
			...
			<div
				...
				{isBgTransparent ? 'dark:text-red' : 'dark:text-text'}"
			>
				{#each navLinks as link}
					<a class="hover:underline active:text-indigo-1000" href="#{link.link}">{link.text}</a>
				{/each}
			</div>
